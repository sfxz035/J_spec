...
#    train_op = tf.train.MomentumOptimizer(learning_rate, 0.9).minimize(mse)
    ...
    summary_op_train = tf.summary.merge([
        ...
        tf.summary.image('train_output', tf.expand_dims(Y_[:,:,:,0],-1)),
        tf.summary.image('train_labels', tf.expand_dims(Y[:,:,:,0],-1)),
        tf.summary.scalar('lr', lr)
    ])
    summary_op_test = tf.summary.merge([
        tf.summary.scalar('test_loss', mse),
        tf.summary.image('test_output', tf.expand_dims(Y_[:,:,:,0],-1)),
    ...
    while True:
        ...
        if (idx) % step_view == 0:
            ...
            for j in range(20):
                ...
                temp_loss = test_pipeline['test_loss']
                test_tot_loss = test_tot_loss + temp_loss
            test_tot_loss = test_tot_loss / 20
            print('step: %d, train loss %.8f, validate loss: %.8f, learning rate: %.8f' % (
            idx, train_tot_loss, test_tot_loss, train_op['lr']))
            train_tot_loss = 0
            test_tot_loss = 0
...
# load data
    ...
    try:
        while True:
            ...
            scio.savemat(
                osp.join(output_dir, 'result' + str(idx) + '.mat'),
