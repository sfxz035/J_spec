...
def conv_bn(inpt ,output_dim, k_h = 1, k_w = 3, strides = [1, 1, 1, 1], is_train = True, name='Conv1d'):
    with tf.variable_scope(name):
        filter_ = weight_variable([k_h,k_w,inpt.get_shape()[-1],output_dim])
        conv = tf.nn.conv2d(inpt, filter=filter_, strides=strides, padding="SAME")
        batch_norm = tf.contrib.layers.batch_norm(conv, scale=True, is_training=is_train, updates_collections=None)
    return batch_norm
...
def conv_relu(inpt, output_dim, k_h = 1, k_w = 3, strides = [1, 1, 1, 1],name='Conv1d'):
    with tf.variable_scope(name):
...
def conv(inpt, output_dim, k_h = 1, k_w = 3, strides = [1, 1, 1, 1],name='Conv1d'):
    with tf.variable_scope(name):
        filter_ = weight_variable([k_h, k_w, inpt.get_shape()[-1], output_dim],name='weights')
        conv = tf.nn.conv2d(inpt, filter=filter_, strides=strides, padding="SAME")
        biases = bias_variable(output_dim,name='biases')
...
def Deconv2d(
        value, output_shape, k_h = 1, k_w = 3, strides =[1, 2, 2, 1],
